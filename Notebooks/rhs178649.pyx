...
cdef complex spectral0(double w, double t): return 0.15 * (w >= 0)
cdef complex spectral1(double w, double t): return 0.15 * (w >= 0)
...
def cy_td_ode_rhs(
    ...
    cop_super_mult(C0, evecs, eig_vec, 1, out, nrows, 1e-12)
    ...
    cdef complex[::1, :] Ac1 = farray_alloc(nrows)
    dense_add_mult(Ac1, A1, interp(t, 0.0, 25.0, spline0))
    dense_add_mult(Ac1, A2, interp(t, 0.0, 25.0, spline0))
    br_term_mult(t, Ac1, evecs, skew, dw_min, spectral1, eig_vec, out, nrows, 1, 0.1, 1e-12)
    PyDataMem_FREE(&Ac1[0,0])
